...
namespace Lambdas
{
	class Lambdas
	{
		static void Main(string[] args)
		{
			...
			var filteredPersonsAverageAge = filteredPersons
				.Select(f => f.Age)
				.Average();
			...
			var personNameMaxLength = persons
				.Max(p => p.Name.Length);

			foreach (KeyValuePair<string, double> kvp in personsByName)
			{
				Console.WriteLine("Имя: " + kvp.Key.PadRight(personNameMaxLength) + " Средний возраст: " + kvp.Value);
			}
